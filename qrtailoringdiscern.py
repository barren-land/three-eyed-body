...
def find(image,image_name,contours,hierachy,root=0):
    ...
    #cv2.drawContours(image,contours,rec[i][6],(255,0,0),3)
    #cv2.drawContours(image,contours,rec[j][6],(255,0,0),3)
    #cv2.drawContours(image,contours,rec[k][6],(255,0,0),3)
    #box内已经有所有位置的元素了
    Xs = [i[0] for i in box]
    Ys = [i[1] for i in box]
    x1 = min(Xs)
    ...
    if x3 < 0:
        y3 = y1
        x3 = x1
    #距离和位置都加30确保图片不会切的刚刚好
    cropImg = image[y3:y1+hight+30, x3:x1+width+30]
    # 初始化cv2的二维码检测器
    detector = cv2.QRCodeDetector()
    qrtxt = ""
    # 解码
    
    data, bbox, straight_qrcode = detector.detectAndDecode(cropImg)
   
    if bbox is not None:
        qrtxt = data
    if data is not None:
        cv2.imwrite(image_name,cropImg)
        #img = cv2.imread("box.jpg")
        reader = zxing.BarCodeReader()
        barcode = reader.decode(image_name)
        qrtxt = barcode.parsed
